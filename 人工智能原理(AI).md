...
### 为什么？
因为先入队的目标不一定是最优的。可能有一个 $f$ 值更大但实际代价更小的目标还在队列里。先入队的相当于是找到的第一个答案，有可能有更优的答案！
```txt
1. 把起点 S 入队
2. 循环：
...
### 二元 CSP
> **二元 CSP**：由三元组 {变量集合 X, 值域集合 D, 约束集合 C} 构成每个约束最多涉及两个变量。
...
function AC-3(csp) returns the CSP, possibly with reduced domains
    ...
    while queue is not empty do // 队列里面还有弧，继续检测
        ...
        if REMOVE-INCONSISTENT-VALUES(Xᵢ, Xⱼ) then // 删掉Xᵢ中不一致的值(Xi, Xj)，若为 true，则：
            for each Xₖ in NEIGHBORS[Xᵢ] do
                add (Xₖ, Xᵢ) to queue // 把弧(Xₖ → Xᵢ)重新加入队列，因为修改Xi后(Xk,Xi)不一定Consistent
...
### 复杂度
- 运行时间：$O(n²d³)$，可优化到 $O(n²d²)$，n：CSP总变量数，d：max{|X1|, |X2|, ..., |Xn|}
...
> 选择*值域最小*的变量先赋值。也叫 "最受约束变量" 启发式 (Most Constrained Variable)。
...
| 策略        | 作用          | 目标        |
| --------- | ----------- | --------- |
| **MRV**   | 选*值域最小*的变量  | 尽早失败，快速剪枝 |
| **度启发式**  | 选*约束最多*的变量  | 快速传播约束    |
...
### 单智能体搜索
- 只有一个玩家
- 状态价值 = 后继状态的最大值：V(s) = max<sub>s'∈children(s)</sub> V(s')
...
| ---------------------------------------------- | ------------------------------------ | ---------------- | ---------------------------------------------------- | ------ | ----------- | ------------------------- |
| **DFS**<br><br>（深度优先搜索）                        | $O(b^m)$                             | $O(bm)$          | b = 分支因子，m = 最大深度。最坏情况遍历整棵树。栈中保存每层的兄弟节点。             | ❌ 不完备  | ❌ 不最优       | 内存有限、解较深、不要求最优解的场景        |
| **BFS**<br><br>（广度优先搜索）                        | $O(b^s)$                             | $O(b^s)$         | s = 最浅解的深度。逐层扩展，最浅解以上的所有节点都要扩展。队列保存最后一层节点。           | ✅ 完备   | ⚠️ 条件最优     | 找最短路径（步数最少）、解的深度不大的场景     |
| **UCS**<br><br>（一致代价搜索）                        | $O(b^{C^{∗}/ε})$                     | $O(b^{C^{∗}/ε})$ | C\*= 最优解代价，ε= 最小边代价。按代价递增的 "等高线" 扩展，有效深度 = C*/ε。     | ✅ 完备   | ✅ 最优        | 边代价不同、需要找代价最小路径的场景        |
| **Greedy Search**<br><br>（贪婪搜索）                | $O(b^m)$                             | $O(bm)$          | 最坏情况和 DFS 一样。启发式不好时可能走极端路径。                          | ❌ 不完备  | ❌ 不最优       | 需要快速找到一个解、不要求最优的场景        |
...
| **Minimax**<br><br>（极小极大）                      | $O(b^m)$                             | $O(bm)$          | b = 分支因子，m = 游戏深度。完整搜索整棵游戏树。递归栈保存路径。                 | ✅ 完备   | ✅ 最优        | 小规模零和游戏、对手最优的对抗场景         |
| **Alpha-Beta Pruning**<br><br>（Alpha-Beta 剪枝）  | 最坏 $O(b^m)$<br><br>完美排序 $O(b^{m/2})$ | $O(b^m)$         | 剪掉不影响结果的分支。好的排序能让搜索深度翻倍。空间和 Minimax 一样。              | ✅ 完备   | ✅ 最优        | 中等规模零和游戏、国际象棋等需要加速的场景     |
...
$$ Q^*(s,a) = \sum_{s'} T(s,a,s') \big[ R(s,a,s') + \operatorname{\gamma} V^*(s') \big] $$
...
> Q 值 = 即时奖励 + 折扣后的未来最优值，按概率加权平均

#### 3、合并后的 Bellman 方程

$$ V^*(s) = \max_a \operatorname{\sum}_{s'} T(s, a, s') \big[ R(s, a, s') + \operatorname{\gamma} V^*(s') \big] $$
...
**初始状态**：
- $V^\pi(A) = 0, V^\pi(B) = 0, V^\pi(C) = 0, V^\pi(D) = 8, V^\pi(E) = 0$

**第一次转移**：{B, east, C, -2}
- $V^\pi(B) \leftarrow (1-0.5) \times 0 + 0.5 \times [-2 + 1.0 \times 0] = -1$

**第二次转移**：{C, east, D, -2}
- $V^\pi(C) \leftarrow (1-0.5) \times 0 + 0.5 \times [-2 + 1.0 \times 8] = 0.5 \times 6 = 3$

#### TD 学习的特点
| 特点           | 说明                                     |
| ------------ | -------------------------------------- |
| **每步更新**(同步) | 每次转移都更新，不需要等整个片段结束                     |
| **利用状态关系**   | 利用了 $V^\pi(s)$ 和 $V^\pi(s')$ 的联系，数据效率高 |
...
> $Q$值： $Q(s, a) \leftarrow Q(s,a)+\alpha \times difference$
